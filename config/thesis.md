...
*Geography:* the **market is the US** (529s, UTMA/UGMA, gift/estate tax, AFR intra-family loans, Medicaid planning — the ~$84T great wealth transfer); the **supply is Europe-preferred** (source European-founded builders pointed at the US surface). *Guardrail:* this decomposes into a cluster of separate agent cells (see the Generational / Lifecycle cluster in the gap map) — it is emphatically **not** a "family-office-OS" super-app, which the global hard filter rejects. A candidate trying to be the whole cluster at once gets the archetype-overlap downweight, not a bonus.
