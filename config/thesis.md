...
*Last updated: 2026-05-22 | Version: 3.7 — Studio thesis settled. Three focus projects (interface layer, intelligent tax, decomposed family office); Nostradamus retired (absorbed by the orchestrator). The orchestrator (Life-Event Planning, G6) elevated as the spine. The generational cluster reorganised around what money does (earns, rests, invests, borrows, protected, saved, spent, orchestrated), adding the earn/rest/spend cells and borrow/protect refinements as new search targets. Prior: Version 3.6 — Studio generational integration. The reviewed "decomposed family office" thesis is now fully integrated: Studio focus area 4 (generational wealth, full lifecycle) added, and an 11-cell Generational / Lifecycle cluster (G1–G11) added to the gap map. US-market / Europe-supply geography; anti-super-app guardrail in force. All 11 cells are blind spots (0 pipeline coverage) → mandatory scan this cycle. Prior context: Version 3.5 — Cycle 21. No new votes since cycle 20 (corpus stable at 97; Turso unreachable from sandbox, worked from cached corpus). No preference or near-miss changes. The cycle's substantive signal is structural: the Phase 0 coverage audit shows underrepresented categories (<3 pipeline companies) have fallen 11 → 9 since the cycle-17 audit. S3 Trade Structuring (now 3 — tradefeedr-adjacent + Range/Zeplyn carries) and R3 Tokenization (now 4) have graduated from underrepresented to adequately covered. The 9 still-underrepresented categories — S2, S5, S6, S7, S8, S9, S10, P5 Audit & Explainability, R1 Retail Derivatives — are elevated to mandatory ≥2-search allocation in Phase 4 this cycle. No 0-company blind spots. All v3.4 architecture intact.*
...
*Vote confirmation (2026-05-13):* ANote Music 5★ (no note) — European music-royalty marketplace with issuance, marketplace, and secondary inside a regulated EU vehicle. First upvote anywhere in the R3 Tokenization cell and the first vote-confirmed entry on the music-IP surface this mandate explicitly names. The discovery log flags a tokenization cluster forming (Wealthyhood, ANote Music, TYTL, Noise) — a dedicated R3 sweep is the standing next-cycle action.
...
**1. The interface layer.** The human-facing layer that makes the orchestrator usable: how an ordinary person interacts efficiently with an always-on system managing their whole financial life. Voice is one modality, but the layer is explicitly **multi-modal**, spanning text, rich visuals, and even 3D. Its job is to turn intent into action and to surface what the orchestrator is doing and proposing in a form a person can grasp, trust, and steer. It is legibility and control over something complex, not a chat box. This is the front door to the orchestrator in focus project 3.
*Reference signals:* frontier-model providers' work on voice and multi-modal interfaces in regulated environments; the internal voice prototype as a starting point.

**2. Intelligent tax.** US financial products are complex; direct indexing is the cleanest case, where you hold individual top constituents so investment losses can offset capital gains. Tax-loss harvesting is not a per-position problem; it requires a holistic view across the client's full account picture, and tax varies state by state. The hypothesis is an intelligence layer, an "accountant in your pocket," running state-by-state harvesting logic on top of the Greens' tax APIs. Tax filing alone is not interesting (juno 2★ is the canonical anti-pattern); portfolio-integrated, real-time tax intelligence is.
*Reference signals:* Alomana (autonomous portfolio rebalancing + TLH, 5★) on the agent side; the Greens' existing tax APIs on the rails side.

**3. The decomposed family office — generational wealth, full lifecycle.** Emulate what a full-service family office does for the very wealthy, break it into its constituent jobs, and democratise each as an agent-callable service so an ordinary household gets the same machinery. Three framings make it novel rather than a robo-advisor rebrand. *The household is an SME with no finance function:* managing a family's full financial life is as complex as running a small business, but the only tool today is an episodic advisor, so the opening is a continuous, always-on finance function. *People are a network, not isolated accounts:* money flows through partners, godparents, communities, employer benefits, and state and federal programs, so the substrate must read the family as part of a wider social and societal fabric. *Money moves both ways:* wealth flows down (parents fund children) and up (adult children support and care for aging parents); the sandwich generation does both at once. The lens is the full lifecycle, from birth and college through a first home, marriage, illness, caring for a parent, retirement and inheritance, and the system's job is to watch for each moment and act ahead of it. *Worked example:* "set aside $100/month for my child from birth" is a living decision the system owns. It opens the right vehicle (529, custodial, or trust), automates it, captures gift-tax-free room, pulls in a contributing grandparent, and re-points the pot when the goal shifts, compounding about $21.6k of contributions to roughly $39k by age 18 at 6%, managed every month for eighteen years.
*Geography:* the target **market is the US** (529s, UTMA/UGMA, gift and estate tax, AFR intra-family loans, Medicaid planning, against the ~$84T great wealth transfer); the **supply is Europe-preferred**. The scout bar is **European-built AND a model that would work in the US in principle**, not "already serving US customers." A European company running on ISA/JISA today qualifies if its approach transfers to US instruments; US-instrument vocabulary is the search target, not a gate. Europe-preferred is not Europe-only; a strong US-built fit is a market-right context signal, not an automatic drop. *Guardrail:* this decomposes into a cluster of separate cells (see the Generational / Lifecycle cluster in the gap map). It is emphatically **not** a "family-office-OS" super-app, which the global hard filter rejects. A candidate trying to be the whole cluster at once gets the archetype-overlap downweight, not a bonus.

**The orchestrator is the spine.** If the thesis decomposes the family office into separate jobs, the orchestrator (Life-Event Planning, G6) is what puts it back together: the continuous, always-on household finance function that sits on the family graph, anticipates the next life moment, and coordinates the right jobs ahead of time, in both directions across the family. The interface layer (focus project 1) is how a person sees and steers it; the single-job specialists are what it calls. Everything else composes around it, which makes it the top priority of the search. (It absorbs what the retired "Nostradamus" prediction project was reaching for, and is distinct from the platform-layer Agent Orchestration cell, which is technical multi-agent routing.)

*What money does — the search structure.* We search for state-of-the-art AI in all forms (agents among them) applied to what a household's money does. It **earns** (income & cashflow, compensation & equity, benefits capture, plus building the next generation's earning capacity), **rests** (cash & yield, liquidity timing, emergency buffer, deposit optimisation), **invests** (the existing specialist cells, plus asset location and goal-linked investing), **borrows** (family lending, mortgage & refinancing, debt payoff, credit access), is **protected** (protection, property/casualty/liability, estate & succession, eldercare, caregiving, health & medical-cost, fraud & security), is **saved** (goal-funding, novel saving mechanisms, gifting & estate-tax), is **spent** (tax-aware drawdown and major-purchase decisions; everyday budgeting is out of scope), and is **orchestrated** by the spine above (the social and societal graph, consent and permissions, and the orchestrator). The earn, rest and spend jobs are the newest search surface; the orchestrator is the priority.
...
| Trade Structuring (S3) | Account selection, options/equity | Asset location, after-tax routing, structure-aware advisor copilot | ⚠️ Adequately covered (3 pipeline cos as of cycle 21) — graduated from underrepresented; maintenance scan |
...
### Generational / Lifecycle cluster (Studio) — organised by what money does

The decomposed family office. Market = US, supply = Europe-preferred. Separate cells, never a super-app. The orchestrator is the spine; the earn, rest and spend jobs are the newest search surface.

| Money job | Search targets (cell codes in brackets) |
|---|---|
| **Earns** | Income & Cashflow (G-INCOME) · Compensation & Equity (G-COMP) · Benefits Capture (G-BENEFITS) · Rising-Generation (G11) |
| **Rests** | Cash & Yield (G-CASH) · Liquidity Timing (G-LIQUIDITY) · Emergency Buffer + Deposit Optimisation (G-DEPOSIT) |
| **Invests** | Investment Intelligence (existing S1–S10) · Asset Location (G-LOCATION) · Goal-Linked Investing (G-GLIDEPATH) |
| **Borrows** | Family Lending (G5) · Mortgage & Refinancing (G-MORTGAGE) · Debt Payoff (G-DEBT) · Credit Access (G-CREDIT) |
| **Is protected** | Protection (G7) · Property/Casualty/Liability (G-PC) · Estate & Succession (G3) · Eldercare & Longevity (G8) · Caregiving & Sandwich-Gen (G9) · Health & Medical-Cost (G10) · Fraud & Financial Security (G-FRAUD) |
| **Is saved** | Goal-Funding (G4) · Novel Saving Mechanisms (G-SAVE) · Gifting & Estate-Tax (G2) |
| **Is spent** | Tax-Aware Drawdown (G-DRAWDOWN) · Major-Purchase Decisions (G-PURCHASE) — everyday budgeting is out of scope |
| **Is orchestrated** (the spine) | Social & Societal Graph (G1) · Consent & Permissions (G-CONSENT) · Life-Event Planning / the orchestrator (G6) |

*Intra-family delegation/consent* is treated as an extension of the existing **Agent Authentication (KYA)** cell (Vouched 5★), not a separate generational cell — flagged for a final placement call. *Cross-references:* G6 extends S8 (Financial Planning); G11 extends S9 (Behavioural Coaching); G2 calls the existing Tax Optimisation cell as a rail.
...
## Learned preferences (97 entries — 79 voted: 20 ★★★★★ · 12 ★★★★ · 9 ★★★ · 16 ★★ · 22 ★ · 18 0★ outreach markers)
...
- *Stage gate fires too late (cycle 18).* Resistant-AI was carded and voted a genuine 5★ on thesis-fit, then killed as a deal on 2026-05-13 — "too late stage … should have caught it earlier." Distinct from the novelty-flavoured walkback (autonomous): here the thesis-fit was real and the stage problem was simply not caught upstream. Sample-of-one alongside linx-security (94→2★, which at least downvoted). Operational implication: apply the Series-B+/over-funded check at carding/radar time, not just as a scoring penalty surfaced post-vote.
- *Consultancy-vs-product concern at human-review stage (cycle 20, n=2).* Alomana 5★ deal-stage softening ("feels more like a consultancy at the moment") and Deepflow 3★ vote-stage note ("looks more like an orchestration tool with some consultancy built in") share the same qualitative signal: the company looks more like services than product when a human inspects it, regardless of carding-stage score. Different funnel stages, different star outcomes, same complaint shape. Distinct from the radar→outreach walkback pattern (which is about score prediction, not business-model framing). Companion observation: both companies presented limited public product surface area at carding time, suggesting a scout-side counter — when card evidence leans on press/funding and the public product surface is thin, flag "consultancy-risk" so reviewers know what to probe for.
...
4. **AI-level safety commoditisation.** Goodfire ($1.25B unicorn) received 2★. The AI interpretability / model safety layer may be commoditised. The surviving opportunity is agent-specific security (Keycard 5★, Resistant-AI 5★). *Update 2026-05-13:* Resistant-AI's 5★ on the angle stands ("Novel and love the agentic security angle"), but the company was killed as a deal on stage — "too late stage … should have caught it earlier." The category survives; the specific company no longer does. The miss is a process signal: the stage gate must fire at carding, not as a post-vote scoring penalty.
...
| 3.2 | 2026-05-13 | Cycle 17 — coverage audit detects 11 underrepresented categories | No thesis structural change. Vote corpus stable at 89 strategic votes (+1 new 0★ outreach marker). Phase 0 audit confirms 11 categories with 1–2 pipeline companies — S2, S3, S5, S6, S7, S8, S9, S10, P5 Audit & Explainability, R1 Retail Derivatives, Tokenization — all elevated to mandatory ≥2-search allocation in Phase 4 this cycle. No 0-company blind spots remain. Continues the operational backlog work declared in v3.0; thesis content unchanged. |
| 3.3 | 2026-05-15 | Cycle 18 — 5 new vote entries (1 voted, 4 outreach) | ANote Music 5★ — first vote-confirmed entry on the R3 Tokenization / music-IP surface; added to Research sub-area 3 reference signals. Resistant-AI re-vote: 5★ on the agentic-security angle stands but the company killed as a deal on stage ("should have caught it earlier") — challenge signal #4 annotated; new tracked pattern "stage gate fires too late" added. 4 new 0★ outreach markers (serveusai, platform1online, contextbases, openbox) — operational, non-strategic. Vote corpus 89 → 94. All v3.2 architecture intact. |
| 3.4 | 2026-05-19 | Cycle 20 — 3 new vote entries (2 voted, 1 outreach) | Greenboard 4★ (US Series A securities compliance, broker-dealers/RIAs, conversational AI layer) — third confirming upvote in "Compliance with Differentiation" alongside Complyance 5★ and Norm-AI 4★; signal stays the same shape (differentiated finserv-specific compliance), evidence base hardens. Deepflow 3★ (UK NatWest Fintech Programme cohort, financial-crime agentic orchestration) — note flags "orchestration tool with some consultancy built in"; combined with Alomana's deal-stage "feels more like a consultancy" softening, new tracked pattern "consultancy-vs-product concern at human-review stage" logged at n=2. Empath-AI 0★ outreach marker (UK NatWest cohort, vocal-biomarker vulnerability detection) — operational, non-strategic. Vote corpus 94 → 97. No gap-map structural changes; all v3.3 architecture intact. |
| 3.5 | 2026-05-20 | Cycle 21 — coverage progress, no new votes | No new votes since cycle 20 (corpus stable at 97; Turso unreachable from sandbox, worked from cached corpus). No preference/near-miss changes. Phase 0 audit shows underrepresented categories fell 11 → 9 since cycle 17: S3 Trade Structuring (3) and R3 Tokenization (4) graduated to adequate coverage. S3 gap-map status updated from ❌ Empty to ⚠️ Adequately covered. The 9 remaining underrepresented categories (S2, S5, S6, S7, S8, S9, S10, P5 Audit & Explainability, R1 Retail Derivatives) elevated to mandatory ≥2-search Phase 4 allocation. No 0-company blind spots. All v3.4 architecture intact. |
| 3.6 | 2026-05-21 | Studio generational integration | The reviewed "decomposed family office" thesis fully integrated. Studio focus area 4 added (generational wealth, full lifecycle: household-as-SME continuous finance function, social & societal graph, money flows both ways, US-market / Europe-supply, anti-super-app guardrail, the $100/month worked example). New 11-cell **Generational / Lifecycle cluster (G1–G11)** added to the gap map — Social & Societal Graph, Gifting & Estate-Tax, Estate & Succession, Goal-Funding, Family Lending, Life-Event Planning (orchestrator), Protection, Eldercare & Longevity, Caregiving & Sandwich-Gen, Health & Medical-Cost, Rising-Generation. All 11 are 0-company blind spots → mandatory scan. Intra-family consent folded into the KYA cell. G6→S8, G11→S9, G2→Tax cross-references noted. Integrated and scouted in the same cycle on the new Exa-first hybrid discovery engine. |
