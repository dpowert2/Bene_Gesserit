...
**Radar→outreach walkback (cycle 15 — first occurrence)**
- autonomous 0★ (2026-05-07: "Potential white label? YC personal investment is promising. Otherwise style over substance right now.") — Autonomous was the headline cycle 9 find, scored BG 92 multi-fit across S-Front + S2 + S8 + Tax. Re-encounter at outreach has walked back the prediction substantially: the recommendation has narrowed to "potential white label" and the qualitative read is "style over substance." Sample-of-one. **Pattern to watch:** a high-confidence radar prediction can fail at outreach when product novelty is weaker than raise-size + accelerator-pedigree signals suggest. If a second case appears (e.g. another BG Score 85+ multi-fit candidate that walks back at outreach), the scoring rubric likely over-weights raise + pedigree relative to product novelty for multi-fit candidates and should be re-balanced.
...
**R3 Tokenization / music-IP surface — first vote confirmation (cycle 18 — 1 occurrence)**
- anote-music 5★ (2026-05-13, no note) — European music-royalty marketplace (issuance + marketplace + secondary inside a regulated EU vehicle), R3 Tokenization. First upvote anywhere in the R3 cell and the first vote-confirmed entry on the music-IP surface that Research mandate #3 explicitly names. No qualitative note, so it reads as a clean confirmation of the category rather than a new direction. The discovery log flags a tokenization cluster forming (Wealthyhood, ANote Music, TYTL, Noise). Tracking — a second R3 upvote would graduate this to a formal positive signal; for now it confirms R3 Tokenization is a real voteable-positive category, not just a coverage-audit gap.
