...
*Last updated: 2026-04-29 | Version: 2.6 — Research team mandate expanded to three sub-areas: (1) Consumer behaviour & market analysis across financial / physical / mental health (Caroline / Matt / Molly — already in v2.4); (2) Investable Everything — Product Inversion & Access, Tokenization & Fractionalization (Caroline Federal); (3) **Wealth-Generation Reinvention** — anything that meaningfully reimagines wealth management or investing, unusual takes on the category, leaps forward from the usual. v2.4 had only sub-area (1); v2.5 added (2); v2.6 closes the loop by adding (3). Reference archetypes: Astor / Era / Autonomous (AI-native RIA / discretionary), Asseta AI (family office OS), Marloo (AI-native advisor platform), Range / Savvy Wealth (broad-mandate wealth platforms), Willow Wealth (retail private-markets benchmark), Seeds (personalised investing reinvention).*
...
8. **Vote pipeline bottleneck (NEW, Cycle 9)** — Supabase has been unreachable from the orchestrator sandbox for FIVE consecutive cycles (cycle 5 onward). The 61-vote preference corpus is frozen; new votes Dave casts in the BG Portal are being recorded server-side but never reach the learning loop. Practical effect: Phase 1 has carried forward unchanged for 5 cycles. The preference model is stale relative to whatever Dave has voted on since 2026-04-15. Action item: investigate sandbox network egress to Supabase or move to a polling job that runs in the user's environment and writes votes.json into the repo.
